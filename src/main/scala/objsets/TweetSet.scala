...
object GoogleVsApple {
  ...
  def inList(t: Tweet, l: List[String]): Boolean = {
    for (s <- l) if (t.text.contains(s)) true
    false
  }
  
  def inGoogle(t:Tweet): Boolean = inList(t, google)
  def inApple(t:Tweet): Boolean = inList(t, apple)
